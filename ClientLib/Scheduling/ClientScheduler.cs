...
public class ClientScheduler {
    ...
    public ClientScheduler(MinecraftClient client) {
        _client = client;
        
        client.Events.AddListener<TickEvent>(_ => {
            // Execute all next tick actions
            if (_tickTasks.Count == 0) return;
            
            ulong tick = _client.CurrentTick;
    ...
    public ScheduledTask ScheduleTask(int ticks, Action action) {
        ...
        _tickTasks.Add((_client.CurrentTick + (ulong)ticks, task));
        return task;
    }
}
