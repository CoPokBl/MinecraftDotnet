...
public class EntityMetaContainer {
    ...
    private static readonly Dictionary<MetaFieldType, Func<MinecraftRegistry, DataReader, MetaField>> FieldTypeReaders = new() {
        ...
        { MetaFieldType.Slot, (reg, reader) => new MetaField<ItemStack>(MetaFieldType.Slot, ItemStack.Read(reader, reg)) },
        ...
        { MetaFieldType.BlockState, (reg, reader) => new MetaField<IBlock>(MetaFieldType.BlockState, reg.Blocks[(uint)reader.ReadVarInt()]) },
        { MetaFieldType.OptionalBlockState, (reg, reader) => new MetaField<Optional<IBlock>>(MetaFieldType.OptionalBlockState, 
            Optional<IBlock>.FromNullable(reader.ReadPrefixedOptional(r => reg.Blocks[(uint)r.ReadVarInt()]))) },
    ...
    private static readonly Dictionary<MetaFieldType, Action<MinecraftRegistry, DataWriter, MetaField>> FieldTypeWriters = new() {
        ...
        { MetaFieldType.Slot, (reg, writer, field) => field.GetValue<ItemStack>().Write(writer, reg) },
